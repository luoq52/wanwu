...
| 参数名     | 必填 | 类型   | 说明                                                         |
| :--------- | :--- | :----- | :----------------------------------------------------------- |
| uuid       | 是   | string | 系统为每个页面请求自动生成的唯一追踪ID，用于全链路日志追踪和问题定位。 |
| parameters | 是   | object | 工作流执行所需的参数对象                                     |

### parameters 对象结构

| 参数名 | 必填 | 类型   | 说明                                   |
| :----- | :--- | :----- | :------------------------------------- |
| input  | 是   | string | 工作流输入参数，示例中为查询的城市名称 |

### 响应参数

| 参数名    | 类型  | 说明                                     |
| :-------- | :---- | :--------------------------------------- |
| forecasts | array | 天气预报数据列表，包含未来多天的天气信息 |

### forecasts

| 参数名          | 类型   | 说明                                  |
| :-------------- | :----- | :------------------------------------ |
| date            | string | 预报日期，格式：YYYY-MM-DD            |
| daypower        | string | 白天风力等级                          |
| daytemp         | string | 白天温度（字符串格式）                |
| daytemp_float   | string | 白天温度（浮点数格式字符串）          |
| dayweather      | string | 白天天气状况                          |
| daywind         | string | 白天风向                              |
| nightpower      | string | 夜间风力等级                          |
| nighttemp       | string | 夜间温度（字符串格式）                |
| nighttemp_float | string | 夜间温度（浮点数格式字符串）          |
| nightweather    | string | 夜间天气状况                          |
| nightwind       | string | 夜间风向                              |
| week            | string | 星期几（数字表示，1-7对应周一到周日） |
...
```shell
curl --request POST \
--location 'http://localhost:8081/service/api/openapi/v1/workflow/run' \
--insecure \

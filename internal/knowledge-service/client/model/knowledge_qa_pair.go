...
type KnowledgeQAPair struct {
	...
	Question     string `gorm:"column:question;type:longtext;not null;comment:'问题'" json:"question"`
	Answer       string `gorm:"column:answer;type:longtext;not null;comment:'答案'" json:"answer"`
	Switch       bool   `gorm:"column:switch;type:tinyint(1);not null;default:0;comment:'开关'" json:"switch"`
	Status       int    `gorm:"column:status;type:tinyint(1);not null;comment:'0-待处理， 1-导入中，2-导入成功，3-导入失败';" json:"status"`
	ErrorMsg     string `gorm:"column:error_msg;type:longtext;not null;comment:'导入状态错误信息'" json:"errorMsg"`
	...
	UpdatedAt    int64  `gorm:"column:update_at;type:bigint(20);not null;autoCreateTime:milli;" json:"updateAt"` // Update Time
